...
def deploy(config_name, cache_dir_path, log, args, cfg):
    ...
    log.info(
        "Started %s deployment at %s on %s"
        % (
            config_path,
    ...
    for recipe in recipes:
        ...
        if (
            ...
            and recipe.get_deep_hash() in open(recipe_cache_path, "r").read()
            and not args.force
        ):
        ...
        log.info(
            "Started %s recipe deployment at %s on %s"
            % (
                recipe.get_name(),
        ...
        log.info(
            "Ended %s recipe deployment at %s on %s"
            % (
                recipe.get_name(),
    ...
    log.success(
        "%s finished in %s" % (config_path, reformat_time(total_deployment_time))
    )
